...
def build_unsafe_tags():
    unsafe_tags = {'dark-web:topic="pornography-child-exploitation"'}
    # add copine-scale tags
    taxonomies = Taxonomies()
    if copine_scale := taxonomies.get('copine-scale'):
...
def is_taxonomie_tag_enabled(taxonomie, tag):
    return tag in r_serv_tags.smembers(f'active_tag_{taxonomie}')

def is_galaxy_tag_enabled(galaxy, tag):
    return tag in r_serv_tags.smembers(f'active_tag_galaxies_{galaxy}')
...
def is_tag_in_all_tag(tag):
    return bool(r_serv_tags.sismember('list_tags', tag))

def get_min_tag(tag):
    tag = tag.split('=')
    if len(tag) > 1:
        tag = tag[1][1:-1] if tag[1] != '' else tag[0][1:-1]
    else:
        tag = tag[0]
    return tag
...
def unpack_str_tags_list(str_tags_list):
    if str_tags_list := str_tags_list.replace('"', '\"'):
...
def get_tag_first_seen(tag, r_int=False):
    ...
    if r_int:
        return 99999999 if res is None else int(res)
    return res
...
def get_tag_last_seen(tag, r_int=False):
    ...
    '''
    res = r_serv_tags.hget(f'tag_metadata:{tag}', 'last_seen')
    if r_int:
...
def get_tag_metadata(tag, r_int=False):
    ...
    '''
    tag_metadata = {"tag": tag, 'first_seen': get_tag_first_seen(tag, r_int=r_int)}
...
def get_tags_min_last_seen(l_tags, r_int=False):
    ...
    return min_last_seen if r_int else str(min_last_seen)
...
def is_obj_tagged(object_id, tag):
    ...
    :rtype: boolean
    '''
    return r_serv_metadata.sismember(f'tag:{object_id}', tag)

def get_all_tags():
    return list(r_serv_tags.smembers('list_tags'))

def get_all_obj_tags(object_type):
    return list(r_serv_tags.smembers(f'list_tags:{object_type}'))
...
def get_obj_tag(object_id):
    ...
    '''
    if res := r_serv_metadata.smembers(f'tag:{object_id}'):
        return list(res)
    else:
        return []
...
def update_tag_first_seen(tag, tag_first_seen, tag_last_seen):
    if tag_first_seen == tag_last_seen:
        if r_serv_tags.scard(f'{tag}:{tag_first_seen}') > 0:
            r_serv_tags.hset(f'tag_metadata:{tag}', 'first_seen', tag_first_seen)
        else:
            r_serv_tags.hdel(f'tag_metadata:{tag}', 'first_seen')
            r_serv_tags.hdel(f'tag_metadata:{tag}', 'last_seen')
    elif r_serv_tags.scard(f'{tag}:{tag_first_seen}') > 0:
        r_serv_tags.hset(f'tag_metadata:{tag}', 'first_seen', tag_first_seen)
    else:
...
def update_tag_last_seen(tag, tag_first_seen, tag_last_seen):
    if tag_first_seen == tag_last_seen:
        if r_serv_tags.scard(f'{tag}:{tag_last_seen}') > 0:
            r_serv_tags.hset(f'tag_metadata:{tag}', 'last_seen', tag_last_seen)
        else:
            r_serv_tags.hdel(f'tag_metadata:{tag}', 'first_seen')
            r_serv_tags.hdel(f'tag_metadata:{tag}', 'last_seen')
    elif r_serv_tags.scard(f'{tag}:{tag_last_seen}') > 0:
        r_serv_tags.hset(f'tag_metadata:{tag}', 'last_seen', tag_last_seen)
...
def update_tag_metadata(tag, tag_date, object_type=None, add_tag=True):
    ...
    if add_tag:
        ...
        if tag_date < tag_metadata['first_seen']:
            r_serv_tags.hset(f'tag_metadata:{tag}', 'first_seen', tag_date)
            # update last_seen
        if tag_date > tag_metadata['last_seen']:
            r_serv_tags.hset(f'tag_metadata:{tag}', 'last_seen', tag_date)
    else:
        if tag_date == tag_metadata['first_seen']:
...
def update_tag_global_by_obj_type(object_type, tag):
    tag_deleted = False
    if object_type=='item':
        if not r_serv_tags.exists(f'tag_metadata:{tag}'):
            tag_deleted = True
    elif not r_serv_tags.exists(f'{object_type}:{tag}'):
        tag_deleted = True
    if tag_deleted:
        # update object global tags
        r_serv_tags.srem(f'list_tags:{object_type}', tag)
        # update global tags
        for obj_type in get_all_objects():
            if r_serv_tags.exists(f'{obj_type}:{tag}'):
                tag_deleted = False
    if tag_deleted:
...
def add_global_tag(tag, object_type=None):
    ...
    if object_type:
        r_serv_tags.sadd(f'list_tags:{object_type}', tag)
...
def api_add_obj_tags(tags=[], galaxy_tags=[], object_id=None, object_type="item"):
    ...
    if res := add_obj_tags(
        object_id, object_type, tags=tags, galaxy_tags=galaxy_tags
    ):
        return res

    res_dict = {'tags': tags + galaxy_tags, 'id': object_id, 'type': object_type}
    return (res_dict, 200)
...
def add_obj_tag(object_type, object_id, tag, obj_date=None):
    if object_type=="item": # # TODO: # FIXME: # REVIEW: rename me !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        # add tag
        r_serv_metadata.sadd(f'tag:{object_id}', tag)
        r_serv_tags.sadd(f'{tag}:{obj_date}', object_id)
        ...
        r_serv_metadata.sadd(f'tag:{object_id}', tag)
        r_serv_tags.sadd(f'{object_type}:{tag}', object_id)
...
def add_tag(object_type, tag, object_id, obj_date=None):
    ...
    r_serv_tags.hincrby(
        f'daily_tags:{datetime.date.today().strftime("%Y%m%d")}', tag, 1
    )

def delete_obj_tag(object_type, object_id, tag, obj_date):
    if object_type=="item": # # TODO: # FIXME: # REVIEW: rename me !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        obj_date = get_obj_date(object_type, object_id)
        r_serv_metadata.srem(f'tag:{object_id}', tag)
        r_serv_tags.srem(f'{tag}:{obj_date}', object_id)
    else:
        r_serv_metadata.srem(f'tag:{object_id}', tag)
        r_serv_tags.srem(f'{object_type}:{tag}', object_id)
...
def api_delete_obj_tags(tags=[], object_id=None, object_type="item"):
    ...
    if res := delete_obj_tags(object_id, object_type, tags=tags):
        return res

    dict_res = {'tags': tags, 'id': object_id}
    return (dict_res, 200)

def delete_obj_tags(object_id, object_type, tags=[]):
    obj_date = get_obj_date(object_type, object_id)
    for tag in tags:
        if res := delete_tag(object_type, tag, object_id, obj_date=obj_date):
            return res
...
def get_obj_keys_by_tags(object_type, l_tags, date_day=None):
    ...
    for tag in l_tags:
        if object_type=='item':
            l_set_keys.append(f'{tag}:{date_day}')
        else:
            l_set_keys.append(f'{object_type}:{tag}')
    return l_set_keys
...
def get_list_of_solo_tags_to_export_by_type(export_type): # by type
    if export_type in ['misp', 'thehive']:
        return r_serv_db.smembers(f'whitelist_{export_type}')
    else:
        return None
