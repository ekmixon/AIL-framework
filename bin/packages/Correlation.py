...
class Correlation(object):
    ...
    def _exist_corelation_field(self, correlation_type, field_name, item_type='paste'):
        if item_type=='paste':
            return r_serv_metadata.exists(
                f'set_{self.correlation_name}_{correlation_type}:{field_name}'
            )

        else:
            return r_serv_metadata.exists(
                f'set_domain_{self.correlation_name}_{correlation_type}:{field_name}'
            )

    def exist_correlation(self, subtype, obj_id):
        res = r_serv_metadata.zscore(f'{self.correlation_name}_all:{subtype}', obj_id)
    ...
    def get_correlation_first_seen(self, subtype, obj_id, r_int=False):
        res = r_serv_metadata.hget(
            f'{self.correlation_name}_metadata_{subtype}:{obj_id}', 'first_seen'
        )
    ...
    def get_correlation_last_seen(self, subtype, obj_id, r_int=False):
        res = r_serv_metadata.hget(
            f'{self.correlation_name}_metadata_{subtype}:{obj_id}', 'last_seen'
        )
    ...
    def _get_metadata(self, subtype, obj_id):
        ...
        meta_dict['nb_seen'] = r_serv_metadata.scard(
            f'set_{self.correlation_name}_{subtype}:{obj_id}'
        )
    ...
    def get_nb_object_seen_by_date(self, correlation_type, field_name, date_day):
        nb = r_serv_metadata.hget(
            f'{self.correlation_name}:{correlation_type}:{date_day}', field_name
        )
    ...
    def get_list_nb_previous_correlation_object(self, correlation_type, field_name, numDay):
        return [
            self.get_nb_object_seen_by_date(correlation_type, field_name, date_day)
            for date_day in Date.get_previous_date_list(numDay)
        ]

    def _get_correlation_by_date(self, correlation_type, date):
        return r_serv_metadata.hkeys(
            f'{self.correlation_name}:{correlation_type}:{date}'
        )
    ...
    def is_valid_obj_subtype(self, subtype):
        return subtype in self.all_correlation_types

    def get_correlation_obj_type(self):
        return 'pgp' if self.correlation_name=='pgpdump' else 'cryptocurrency'
    ...
    def sanythise_correlation_types(self, correlation_types, r_boolean=False):
        ...
        if correlation_types is None:
            return False if r_boolean else self.get_all_correlation_types()
        for correl in correlation_types: # # TODO: # OPTIMIZE:
            if correl not in self.get_all_correlation_types():
                return False if r_boolean else self.get_all_correlation_types()
        return True if r_boolean else correlation_types
    ...
    def _get_domain_correlation_obj(self, domain, correlation_type):
        ...
        '''
        if res := r_serv_metadata.smembers(
            f'domain_{self.correlation_name}_{correlation_type}:{domain}'
        ):
            return list(res)
        else:
            return []
    ...
    def get_domain_correlation_dict(self, domain, correlation_type=None, get_nb=False):
        ...
        for correl in correlation_type:
            if res := self._get_domain_correlation_obj(domain, correl):
    ...
    def get_correlation_obj_domain(self, field_name, correlation_type=None):
        ...
        correlation_type = self.sanythise_correlation_types(correlation_type)
        for correl in correlation_type:
            if res := self._get_correlation_obj_domain(field_name, correl):
                return res
        return []
    ...
    def _get_item_correlation_obj(self, item_id, correlation_type):
        ...
        '''
        if res := r_serv_metadata.smembers(
            f'item_{self.correlation_name}_{correlation_type}:{item_id}'
        ):
            return list(res)
        else:
            return []
    ...
    def get_item_correlation_dict(self, item_id, correlation_type=None, get_nb=False):
        ...
        dict_correlation = {}
        for correl in correlation_type:
            if res := self._get_item_correlation_obj(item_id, correl):
                dict_correlation[correl] = res
                if get_nb:
                    dict_correlation['nb'] = dict_correlation.get('nb', 0) + len(dict_correlation[correl])
        return dict_correlation
    ...
    def update_correlation_daterange(self, subtype, obj_id, date):
        ...
        if not r_serv_metadata.exists(
            f'{self.correlation_name}_metadata_{subtype}:{obj_id}'
        ):
    ...
    def save_item_correlation(self, subtype, obj_id, item_id, item_date):
        ...
        r_serv_metadata.sadd(
            f'set_{self.correlation_name}_{subtype}:{obj_id}', item_id
        )


        # daily
        r_serv_metadata.hincrby(
            f'{self.correlation_name}:{subtype}:{item_date}', obj_id, 1
        )


        # all type
        r_serv_metadata.zincrby(f'{self.correlation_name}_all:{subtype}', obj_id, 1)

        ## object_metadata
        # item
        r_serv_metadata.sadd(
            f'item_{self.correlation_name}_{subtype}:{item_id}', obj_id
        )
    ...
    def delete_item_correlation(self, subtype, obj_id, item_id, item_date):
        ...
        res = r_serv_metadata.hincrby(
            f'{self.correlation_name}:{subtype}:{item_date}', obj_id, -1
        )

        if int(res) < 0: # remove last
            r_serv_metadata.hdel(f'{self.correlation_name}:{subtype}:{item_date}', obj_id)
    ...
    def save_domain_correlation(self, domain, subtype, obj_id):
        r_serv_metadata.sadd(
            f'domain_{self.correlation_name}_{subtype}:{domain}', obj_id
        )

        r_serv_metadata.sadd(
            f'set_domain_{self.correlation_name}_{subtype}:{obj_id}', domain
        )
    ...
    def save_correlation(self, subtype, obj_id, date_range):
        r_serv_metadata.zincrby(f'{self.correlation_name}_all:{subtype}', obj_id, 0)
    ...
    def delete_correlation(self, subtype, obj_id):
        ...
        if 'domain' in obj_correlations:
            ...
            r_serv_metadata.delete(
                f'set_domain_{self.correlation_name}_{subtype}:{obj_id}'
            )
        ...
        if 'paste' in obj_correlations: # TODO: handle item
            ...
            r_serv_metadata.delete(f'set_{self.correlation_name}_{subtype}:{obj_id}')
            ...
            for date_day in date_range:
                r_serv_metadata.hdel(f'{self.correlation_name}:{subtype}:{date_day}', obj_id)

        r_serv_metadata.delete(f'{self.correlation_name}_metadata_{subtype}:{obj_id}')
        r_serv_metadata.zrem(f'{self.correlation_name}_all:{subtype}', obj_id)
